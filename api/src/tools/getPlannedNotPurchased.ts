/**
 * getPlannedNotPurchased
 * Retorna médicos que compraram no trimestre anterior mas não compraram
 * no trimestre atual (detectado dinamicamente pelo dado mais recente).
 * Ordenados por segmentação A→E.
 * TODO: plugar tabela de planejamento real aqui quando disponível.
 */
...
export async function getPlannedNotPurchased(): Promise<PlannedNotPurchased[]> {
  ...
  for (const doc of (docData || []) as Array<{ id: string; seg_dysport: string; seg_restylane: string; seg_sculptra: string }>) {
    ...
    .map(oneId => {
      ...
      if (doc) {
        seg = (last.brand === 'DYSPORT' ? doc.seg_dysport : last.brand === 'RESTYLANE' ? doc.seg_restylane : doc.seg_sculptra)
          || doc.seg_dysport || doc.seg_restylane || doc.seg_sculptra
      }
      return { one_name: last.one_name, segmentacao: seg, last_product: last.brand, last_purchase_date: last.billed_at }
    })
    .sort((a, b) => (SEG_ORDER[a.segmentacao || ''] ?? 99) - (SEG_ORDER[b.segmentacao || ''] ?? 99))
}

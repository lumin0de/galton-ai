 ...
 * Retorna médicos da carteira do representante que estão próximos de atingir
 * o critério de conta ativa nos últimos 3 meses. Top 5 por proximidade da meta.
 */
...
export async function getNearActiveAccounts(territoryCode?: string): Promise<NearActiveBySegGroup[]> {
  ...
  // Fetch segmentations by one_id (group account level — same as getDropouts in alerts.ts)
  ...
  // Build result: filter 50% <= pct < 100%
  ...
  // Group by segmentation, top 5 per group, order A→E then N/D

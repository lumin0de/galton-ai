...
function prevQuarter(q: string): string {
  // q = "Q4_2024" → "Q3_2024"; "Q1_2025" → "Q4_2024"
...
// ─── Detect most recent quarter with data ─────────────────────────────────────

async function getLatestQuarter(): Promise<string> {
  // Find max year_ref + month_ref in sales to derive the current quarter
...
async function getDropouts(currentQ: string, previousQ: string, territoryCode?: string) {
  ...
  for (const row of (prevData || []) as Array<{ one_id: string; one_name: string; brand: string; qty_equiv: number; doctor_id: string }>) {
  ...
  // Accounts that were active in previous quarter
  ...
  const dropouts: Array<{ one_name: string; brand: string; prev_qty: number; curr_qty: number; meta: number; segmentacao: string | null }> = []
  ...
  for (const [key, val] of Object.entries(prevAgg)) {
    ...
    seen.add(key)
    dropouts.push({ one_name: val.one_name, brand, prev_qty: Math.round(val.qty * 10) / 10, curr_qty: Math.round(currQty * 10) / 10, meta: target, segmentacao: null })
  }

  // Enrich segmentation
  const oneIds = [...new Set(
    dropouts.map(d => Object.entries(prevAgg).find(([, v]) => v.one_name === d.one_name)?.[0].split('||')[0]).filter(Boolean)
  )] as string[]
  ...
  for (const d of (docs || []) as Array<{ one_id: string; seg_dysport: string; seg_restylane: string; seg_sculptra: string }>) {
...
async function getCrossSell(currentQ: string, territoryCode?: string) {
  ...
  // Build cross-sell list keeping oneId for segmentation lookup
  ...
  // Enrich segmentation by one_id
  const csOneIds = [...new Set(crossSellRaw.map(c => c.oneId))]
  const { data: csDocData } = await supabase
    .from('doctors')
    .select('one_id, seg_dysport, seg_restylane, seg_sculptra')
    .in('one_id', csOneIds)

  const csSegMap: Record<string, { seg_dysport: string; seg_restylane: string; seg_sculptra: string }> = {}
  for (const doc of (csDocData || []) as Array<{ one_id: string; seg_dysport: string; seg_restylane: string; seg_sculptra: string }>) {
  ...
  const crossSell = crossSellRaw.map(({ oneId, ...rest }) => {
    ...
    if (doc) {
      seg = (rest.missing === 'DYSPORT' ? doc.seg_dysport
        : rest.missing === 'RESTYLANE' ? doc.seg_restylane
        : doc.seg_sculptra)
        || doc.seg_dysport || doc.seg_restylane || doc.seg_sculptra || null
    }
...
async function getPlannedNotPurchased(currentQ: string, previousQ: string, territoryCode?: string) {
  ...
  // Last purchase info per one_id from previous quarter
  const lastMap: Record<string, { one_name: string; brand: string; billed_at: string; doctor_id: string }> = {}
  for (const row of prevData as Array<{ one_id: string; one_name: string; brand: string; billed_at: string; doctor_id: string }>) {
    const existing = lastMap[row.one_id]
    if (!existing || (row.billed_at > existing.billed_at)) lastMap[row.one_id] = row
  }

  // Lookup segmentation by one_id (account level, not individual doctor)
  ...
  for (const doc of (docData || []) as Array<{ one_id: string; seg_dysport: string; seg_restylane: string; seg_sculptra: string }>) {
  ...
  return notPurchased
    ...
    .map(oneId => {
      ...
      let seg: string | null = null
      if (doc) {
        seg = (last.brand === 'DYSPORT' ? doc.seg_dysport : last.brand === 'RESTYLANE' ? doc.seg_restylane : doc.seg_sculptra)
          || doc.seg_dysport || doc.seg_restylane || doc.seg_sculptra || null
      }
      // Use billed_at if available, otherwise show the previous quarter as reference
      const dateDisplay = last.billed_at || previousQ
      return { one_name: last.one_name, segmentacao: seg, last_product: last.brand, last_purchase_date: dateDisplay }
    })
    .sort((a, b) => (SEG_ORDER_LOCAL[a.segmentacao || ''] ?? 99) - (SEG_ORDER_LOCAL[b.segmentacao || ''] ?? 99))
}
